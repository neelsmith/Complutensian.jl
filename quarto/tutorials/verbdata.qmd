...
```{julia}
creo = "ls.n11543"
psgsequences = passagesforverb(creo, verbdata)
length(psgsequences)
```
...
```{julia}
urnforpsg(psgsequences[2], verbdata)
```
:::
...
```{julia}
cooccurencescores(creo, verbdata)
```
...
```{julia}
psg84lexx = lexemesforpsg(1, verbdata)
